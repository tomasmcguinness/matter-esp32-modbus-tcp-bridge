    ...
    if (ep0) {
        ...
        if (cJSON_IsArray(parts)) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (part) parse_endpoint_mappings(part, (size_t)(i + 1), out);
            }
        }
    }
    cJSON_Delete(root);
...
{
    if (!json || !json[0]) return MATTER_ENDPOINT_ID_INVALID;
    cJSON *root = cJSON_Parse(json);
...
{
    // Parse root endpoint info
    uint32_t root_device_type = ESP_MATTER_SOLAR_POWER_DEVICE_TYPE_ID;
    int      part_count       = 0;
            ...
            if (ep0) {
                ...
                if (cJSON_IsArray(parts)) {
                    ...
                    for (int i = 0; i < part_count; i++) {
                        ...
                        if (pdt0 && cJSON_IsNumber(pdt0)) pdt = (uint32_t)pdt0->valueint;
    ...
    for (auto &pe : part_entries) {
        if (pe.bridge_dev) endpoint::enable(pe.bridge_dev->endpoint);
    }
    ...
    for (int i = 0; i < part_count; i++) any_new |= part_new_flags[i];
    ...
    // --- Build SolarPowerDevice instances ---
    root_entry.matter = new SolarPowerDevice(root_entry.bridge_dev, config);
    for (auto &pe : part_entries) {
        if (pe.bridge_dev) pe.matter = new SolarPowerDevice(pe.bridge_dev, config);
    }
    ...
    pair.endpoints.push_back(root_entry);
    for (auto &pe : part_entries) pair.endpoints.push_back(pe);
    pair.mappings = all_mappings;
    m_devices.push_back(std::move(pair));

    ESP_LOGI(TAG, "Registered '%s': root ep=%u + %d part(s)",
             config.name, root_ep_id, part_count);
    return ESP_OK;
}
...
esp_err_t MatterManager::init(esp_matter::node_t *node, esp_matter::endpoint_t *aggregator)
{
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (!modbus) {
            ESP_LOGW(TAG, "No Modbus device found for '%s', skipping", cfg->id);
            continue;
        }
    ...
    for (const auto &r : readings) {
        for (const auto &m : it->mappings) {
            if (m.reg_address == r.address && m.endpoint_idx < it->endpoints.size()) {
                SolarPowerDevice *dev = it->endpoints[m.endpoint_idx].matter;
...
bool MatterManager::get_readings(const char *id, DeviceReadings &out) const
{
    ...
    for (const auto &m : it->mappings) {
        if (m.endpoint_idx >= it->endpoints.size()) continue;
        const EndpointEntry &entry = it->endpoints[m.endpoint_idx];
        if (!entry.matter) continue;
        int64_t value = 0;
        if (entry.matter->get_raw(m.cluster_id, m.attribute_id, value)) {
            out.push_back({entry.matter->endpoint_id(), m.cluster_id, m.attribute_id, value});
        }
    }
    return true;
}
...
esp_err_t MatterManager::on_device_removed(const char *id)
{
    ...
    for (auto &entry : it->endpoints) {
        delete entry.matter;
        if (entry.bridge_dev) esp_matter_bridge::remove_device(entry.bridge_dev);
    }
    chip::DeviceLayer::PlatformMgr().UnlockChipStack();
...
uint16_t MatterManager::endpoint_id(const char *id) const
{
    auto it = std::find_if(m_devices.begin(), m_devices.end(),
        [id](const DevicePair &p) { return strcmp(p.id, id) == 0; });
    if (it == m_devices.end() || it->endpoints.empty() || !it->endpoints[0].matter) return 0;
    return it->endpoints[0].matter->endpoint_id();
}
