...
esp_err_t ModbusManager::register_device(const device_config_t &config)
{
    ...
    if (!solar_dev) {
        solar_dev = esp_matter_bridge::create_device(m_node, m_aggregator_endpoint_id,
    ...
    // auto *arg    = new ReadingsArg{solar, pv1, pv2};

    // modbus->set_readings_callback(readings_update_cb, arg);
    // m_devices.push_back({modbus, solar, pv1, pv2, solar_dev, pv1_dev, pv2_dev, arg});
